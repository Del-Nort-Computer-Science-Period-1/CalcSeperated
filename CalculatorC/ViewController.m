...
-(IBAction)equalButton:(id)sender {  // Interface Builder action for equal (calculation)
    ...
    [self setCalcAreaNumber:[NSString stringWithFormat:@"%0.2f", calcAnswer]];  // float (double) to string
    [self setTextCalcAreaLabel];
...
-(IBAction)squaredButton:(id)sender {
    [self saveValueOfOperator:SQUARED];
    [self saveValueOfArg1];
    [self calculateAnswer];
